export default {
  code: "de",
  nativeName: "Deutsch",
  meta: {
    description:
      "Kirill — Frontend-Entwickler. Vue, Nuxt, TypeScript. Profil, Stack und Projekte.",
  },
  ui: {
    pageTitleSuffix: "Profil",
    skipToContent: "Zum Inhalt",
    navLabel: "Hauptnavigation",
    langLabel: "Sprache",
    backToTop: "Nach oben",
    footerNote: "Static profile · GitHub Pages",
    birthPrefix: "geb.",
    bannerAlt: "Banner",
    avatarAlt: "Avatar",
    hobbies: "Hobbys",
    gameGenres: "Lieblingsgenres",
    projectsSubtitle: "Bibliothek — ausgewählte Repositories",
  },
  spoken: {
    ru: "Russisch",
    en: "Englisch",
  },
  hero: {
    role: "Frontend-Entwickler",
    status: "Online",
    tagline:
      "Ich gestalte Interfaces, die gut aussehen und angenehm zu bedienen sind.",
    location: "Remote",
  },
  hub: {
    eyebrow: "Social-Hub",
    title: "Linktree",
    blurb:
      "Frontend-Entwickler mit Fokus auf clean UX und modernes Webdesign — besonders Vue/Nuxt. Ich teile Gedanken zu Gaming und Web-Technologien.",
    cta: "Linktree öffnen",
    platformsLabel: "Auch auf",
    platforms: {
      telegram: "Telegram",
      email: "Email",
      discord: "Discord",
      whatsapp: "WhatsApp",
      facebook: "Facebook",
      github: "GitHub",
      steam: "Steam",
      backloggd: "Backloggd",
      letterboxd: "Letterboxd",
      notion: "Notion",
    },
  },
  letterboxd: {
    title: "Letterboxd",
    filmsWatched: "{count} Filme",
    favoritesLabel: "Favoriten",
  },
  about: {
    title: "Über mich",
    eyebrow: "Frontend-first · Backend auch",
    badgesLabel: "Highlights",
    lead:
      "Ich baue Interfaces, in denen <mark class=\"about-hl\">Technik</mark> und Geschmack zusammenarbeiten — schnell, präzise und detailverliebt.",
    paragraphs: [
      "Ich verbinde Engineering gern mit einem visuellen, kreativen Blick. Mit dem <strong>Backend</strong> komme ich klar (Node.js, PHP, Laravel, Yii2), aber <mark class=\"about-hl\">Frontend</mark> liegt mir mehr — mehr Freude am Ergebnis und stärkerer Fokus auf UX.",
      "In der IT treibt mich an, <strong>Ergebnisse sofort zu sehen</strong>: eine Idee wird zum laufenden Produkt. Modernes AI-Tooling hilft mir, schneller zu iterieren, ohne Codequalität zu opfern.",
    ],
    badges: {
      craft: "UI-Handwerk",
      vue: "Vue · Nuxt",
      ts: "TypeScript",
      backend: "Backend auch",
      ai: "AI-native",
      agents: "KI-Agenten",
      remote: "Remote-ready",
    },
    toolkitLabel: "KI & Agenten",
    toolkitBlurb:
      "Alltags-Stack, um Entwicklung, Review und Design mit neuronalen Netzen zu beschleunigen.",
  },
  focus: {
    title: "Fokus",
    items: [
      {
        id: "ui",
        label: "Interfaces",
        detail:
          "Nützliche, bedienbare UI mit Fokus auf Frontend-Architektur.",
      },
      {
        id: "frameworks",
        label: "Frameworks",
        detail:
          "Arbeit mit modernem Stack: Vue.js, Nuxt.js, TypeScript und sauberem Markup.",
      },
      {
        id: "format",
        label: "Arbeitsweise",
        detail:
          "Ich bevorzuge Remote: Flexibilität, Autonomie und eine angenehme Atmosphäre.",
      },
    ],
  },
  stack: {
    title: "Stack",
    groups: {
      frontend: "Frontend",
      backend: "Backend",
      languages: "Sprachen",
    },
  },
  projects: {
    title: "Projekte",
    status: {
      public: "Public",
    },
    blurbs: {
      "steam-gamestatus":
        "Steam-Store-Userscript: Crack/Denuvo-Status-Badges über die GameStatus.info-API.",
      "youtube-bot-comments-filter":
        "YouTube-Userscript: erkennt Spam-Bots am Nick-Muster und blendet Kommentare aus oder unscharf.",
      ProxyChecker:
        "Plattformübergreifende Desktop-App zum Prüfen von Proxy-Verfügbarkeit und Leistung.",
      "steam-region-block-bypass":
        "Steam-Userscript: stellt die Produktseite wieder her, wenn eine Regionsperre angezeigt wird.",
    },
  },
  interests: {
    title: "Interessen",
    hobbies: ["Videospiele", "Filme", "Anime", "Serien", "Manga"],
    gameGenres: [
      "Story-Shooter",
      "Sandboxes",
      "Postapokalypse / Zombies",
    ],
  },
  links: {
    title: "Links",
    hints: {
      github: "Code & Repositories",
      linktree: "Alle Socials an einem Ort",
      letterboxd: "Filme & Listen",
      orcid: "Wissenschaftliches Profil",
    },
  },
  nav: {
    about: "Über mich",
    focus: "Fokus",
    stack: "Stack",
    projects: "Projekte",
    interests: "Interessen",
    links: "Links",
  },
};
